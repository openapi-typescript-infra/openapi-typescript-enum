...
async function generateSchema(pathToSpec: string | typeof process.stdin) {
  ...
  // generate schema
  const result = await generate(pathToSpec, {
  ...
  if (output === OUTPUT_FILE) {
    ...
    if (isDir) {
      const filename = (pathToSpec as string).replace(EXT_RE, '.ts');
...
async function main() {
  ...
  if (!pathToSpec) {
    if (output === OUTPUT_FILE) fs.mkdirSync(outputDir, { recursive: true });
